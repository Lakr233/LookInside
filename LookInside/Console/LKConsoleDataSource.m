    ...
    if (self = [self init]) {
        ...
        RAC(self, selectedObjects) = [RACObserve(hierarchyDataSource, selectedItem)
                                      map:^id _Nullable(LookinDisplayItem *item) {
                                          ...
                                          NSArray<LookinObject *> *objs = $(item.hostViewControllerObject, item.hostWindowControllerObject, item.layerObject, item.viewObject, item.windowObject, item.kindObject).array;
                                          return objs;
                                      }];
    }
    return self;
...
- (RACSignal *)submitWithObj:(LookinObject *)obj text:(NSString *)text {
    ...
    if ([text containsString:@":"]) {
        ...
        return [RACSignal error:LookinErrorMake(NSLocalizedString(@"LookInside doesn't support invoking methods with arguments yet.", nil), errDesc)];
    }
    if ([text containsString:@"."]) {
