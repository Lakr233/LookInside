...
- (RACSignal *)modifyAttribute:(LookinAttribute *)attribute newValue:(id)newValue {
    if (attribute.isUserCustom) {
        if (attribute.customSetterID.length == 0) {
            // Read-only user-custom attribute (e.g. SwiftUI-injected node attrs
            // produced by LKS_SwiftUIAttrGroupsMaker do not register a setter).
            // The per-attrView controlTextDidEndEditing: guards already reject
            // these via canEdit, but keep this entry-level fallback so any
            // future caller cannot trip the customSetterID assertion below.
        ...
        [[inbuiltInspectableApp submitInbuiltModification:modification] subscribeNext:^(LookinDisplayItemDetail *detail) {
            ...
            if (self.staticDataSource) {
                ...
                if ([LookinDashboardBlueprint needPatchAfterModificationWithAttrID:attribute.identifier]) {
                    [self.asyncUpdateManager updateAfterModifyingDisplayItem:(LookinStaticDisplayItem *)modifyingItem];
                }
