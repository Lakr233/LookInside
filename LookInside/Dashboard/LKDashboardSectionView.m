...
//            self.titleLabel.backgroundColor = [NSColor greenColor];
    ...
    [self.attrSection.attributes enumerateObjectsUsingBlock:^(LookinAttribute * _Nonnull attr, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (!attrViewClass) {
            // Debug builds trip the assert so a missing case in
            // _targetAttrClassForType: surfaces immediately. Release builds
            // continue to the next attribute (NSAssert is a no-op there) and
            // log a breadcrumb so the skipped row is recoverable post-mortem.
            NSLog(@"LookInside dashboard: skipping attribute with unsupported attrType=%ld identifier=%@",
                  (long)attr.attrType, attr.identifier);
            NSAssert(NO, @"LookInside dashboard: no attrView class for attrType=%ld identifier=%@",
                     (long)attr.attrType, attr.identifier);
            return;
        }
...
- (Class)_targetAttrClassForType:(LookinAttrType)type identifier:(LookinAttrIdentifier)identifier {
    // Pure lookup: returns nil for any unsupported attrType / identifier.
    // The single assert lives in the caller (setAttrSection:) so callees that
    // only need a class probe don't pay the debug-time crash.
    switch (type) {
        ...
        case LookinAttrTypeCustomObj:
            if ([identifier isEqualToString:LookinAttr_UITableView_RowsNumber_Number]) {
                ...
                return [LKDashboardAttributeConstraintsView class];
                       ...
                       || [identifier isEqualToString:LookinAttr_NSImageView_Open_Open]) {
