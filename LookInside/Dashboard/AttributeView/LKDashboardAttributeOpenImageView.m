...
- (void)_handleClick {
    ...
    if (!dashController.isStaticMode) {
        AlertErrorText(NSLocalizedString(@"The feature is not available in current mode.", nil), NSLocalizedString(@"You must connect LookInside with target iOS app before using this feature.", nil), self.window);
        return;
    }
