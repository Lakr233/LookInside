...
- (void)viewDidLayout {
    ...
    [$(self.connectionTipsView, self.imageSyncTipsView, self.tooLargeToSyncScreenshotTipsView, self.noPreviewTipView, self.focusTipView, self.userConfigNoPreviewTipsView, self.customViewTipView, self.fastModeTipView).visibles.array enumerateObjectsUsingBlock:^(LKTipsView *tipsView, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)handleCustomViewTipsView {
    [LKHelper showDisabledExternalLinkAlertWithMessage:NSLocalizedString(@"Legacy custom information guides are disabled in this community build. See the repository README instead.", nil)];
}
...
- (void)handleFastModeDocumentation {
    [LKHelper showDisabledExternalLinkAlertWithMessage:NSLocalizedString(@"Legacy fast mode documentation links are disabled in this community build. See the repository README instead.", nil)];
}
