...
+ (void)showDisabledExternalLinkAlertWithMessage:(NSString *)message {
...
+ (void)openLookinWebsiteWithPath:(NSString *)path {
    [self showDisabledExternalLinkAlertWithMessage:NSLocalizedString(@"Legacy website links are disabled in this community build. See the local repository README for current documentation.", nil)];
}

+ (void)openLookinOfficialWebsite {
    [self showDisabledExternalLinkAlertWithMessage:NSLocalizedString(@"A public project website is not configured for this build yet.", nil)];
}

+ (void)openCustomConfigWebsite {
    [self showDisabledExternalLinkAlertWithMessage:NSLocalizedString(@"Legacy documentation links are disabled in this community build.", nil)];
}

+ (void)openProjectGitHubRepository {
    NSURL *url = [NSURL URLWithString:@"https://github.com/Lakr233/LookInside"];
    if (url) {
...
+ (void)openProjectREADME {
    NSString *readmePath = [NSFileManager.defaultManager.currentDirectoryPath stringByAppendingPathComponent:@"README.md"];
    if (![NSFileManager.defaultManager fileExistsAtPath:readmePath]) {
        [self showDisabledExternalLinkAlertWithMessage:NSLocalizedString(@"README.md was not found in the current repository checkout.", nil)];
        return;
    }
